...
public class ClosedPairPoints extends Sorter{
    ...
    public void print() {
        ...
        float time = Timer.timeIn(() ->recursive(createRandomPointList(100000), 0));
        recursive(createRandomPointList(100000), 0);
        System.out.println("closed pair points in recursive: "+time+ "recursion depth:"+recurciveCount+"switch count:"+switchCount);
        //CSVWriter.AppendWriting("CPP recursive",1000, time, recurciveCount, switchCount);
        time = Timer.timeIn(() ->BrutalForce(createRandomPointList(100000)));
